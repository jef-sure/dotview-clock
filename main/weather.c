...
const weather_t* getRandomWeather() {
    const int32_t N = sizeof(openweather) / sizeof(openweather[0]);
    int r = rand() % N;
    return openweather + r;
}

static uint8_t clamp(int rgb) { return rgb < 0 ? 0 : rgb > 255 ? 255 : rgb; }
